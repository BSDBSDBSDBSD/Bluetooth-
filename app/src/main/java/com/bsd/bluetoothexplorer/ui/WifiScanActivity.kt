...
/**
 * Direct Wi-Fi (Wi-Fi Direct) connection to the file server — no router, no internet.
 * The server phone hosts a Wi-Fi Direct group (becomes host at 192.168.49.1). Here we
 * discover it, join as a client, then open a TCP socket to the host. A manual "connect by
 * IP" box stays as a fallback for phones already on the same network.
 */
class WifiScanActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        supportActionBar?.title = "WiFi ישיר"
        ...
        peerAdapter = object : BaseAdapter() {
            ...
            override fun getView(pos: Int, convertView: View?, parent: ViewGroup): View {
                ...
                try { v.background = getDrawable(R.drawable.item_device_bg) } catch (_: Exception) {}
                return v
            }
        }
        ...
        wifi.onPeersChanged = { list ->
            runOnUiThread {
                ...
                if (!connecting) tvHint.text = if (list.isEmpty())
                    "מחפש… במכשיר השני: פתח את האפליקציה ולחץ \"הפעל שרת\""
                else "${list.size} מכשירים נמצאו — בחר מכשיר:"
            }
        }
            ...
            if (connected && info != null && info.groupFormed) {
                if (!info.isGroupOwner) {
                    ...
                    runOnUiThread {
                        ...
                        tvHint.text = "המכשיר הזה הפך למארח בטעות. נתק ונסה שוב מהמכשיר עם השרת."
                    }
                }
            }
        }
        ...
        lifecycleScope.launch {
            ...
            if (!isDestroyed) {
                ...
                if (success) {
                    ...
                    tvHint.text = "❌ לא ניתן להתחבר לשרת.\nוודא שבמכשיר השני מופעל \"הפעל שרת\"."
                }
            }
        }
    }
